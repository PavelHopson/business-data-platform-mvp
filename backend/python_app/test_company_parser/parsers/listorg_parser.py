...
class ListOrgParser(BaseParser):
    ...
    def _extract_company_data(self, soup) -> Dict[str, Any]:
        ...
        if inn_tag:
            inn_span = inn_tag.find_parent('p').find('span', class_='clipboard')
            if inn_span:
                data["inn"] = self.clean_text(inn_span.find('span').text)
        
        ogrn_tag = soup.find('span', {'title': 'Основной государственный регистрационный номер'})
        if ogrn_tag:
            ogrn_span = ogrn_tag.find_parent('p').find('span', class_='clipboard')
            if ogrn_span:
                data["ogrn"] = self.clean_text(ogrn_span.find('span').text)

        name_tag = soup.find(lambda tag: tag.name == 'td' and 'Полное юридическое наименование:' in tag.get_text())
        if name_tag:
            name_link = name_tag.find_next_sibling('td').find('a')
            if name_link:
        ...
        if dir_tag:
            dir_link = dir_tag.find_next_sibling('td').find('a')
            if dir_link:
                data["general_director"] = self.clean_text(dir_link.get_text())

        founders_table = soup.find('table', attrs={'class': re.compile('tt f08m')})
        if founders_table:
            rows = founders_table.find_all('tr')[1:]
            for row in rows:
        ...
        cases_header = soup.find('p', class_='c_c', string=re.compile(r'Судебные дела'))
        if cases_header:
            count = self.extract_number(cases_header.get_text())
            if count:
                data["court_cases"] = count
            else:
                cases_table = cases_header.find_next_sibling('table')
                if cases_table:
                    rows = cases_table.find_all('tr')
                    data["court_cases"] = max(0, len(rows) - 1)
